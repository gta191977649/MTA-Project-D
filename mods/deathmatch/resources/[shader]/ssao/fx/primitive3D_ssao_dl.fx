...
Pixel PixelShaderFunctionAO(PSInput PS)
{
    ...
    float3 viewNormal = 0;
    if (texNormal.z > 0.4999f)
    {
        if (texNormal.z < 0.5999f)
        {
            output.Color = float4(viewNormal.xy, 0, 1);
            output.World = 0;
            return output;
        }
        worldNormal = float3((texNormal.xy - 0.5) * 2, 0);
        worldNormal.z =  1 - length(worldNormal.xy);
        worldNormal.z = fmod(texNormal.z, 0.2) > 0.1 ? worldNormal.z : -worldNormal.z;
        worldNormal = normalize(worldNormal);
        viewNormal = mul(worldNormal.xyz, (float3x3)gView);
    }
    else
    {
        // recreate world normal from depth
        viewNormal = normalize(GetNormalFromDepth(PS.TexCoord.xy, PS.UvToView));
    }
...
Pixel PixelShaderFunctionBlur1(PSInput PS)
{
    ...
    viewNormal.z = length(viewNormal.xy) - 1;
...
// Techniques
//--------------------------------------------------------------------------------------
technique dxDrawPrimitive3D_ssao_dl
{
  pass P0
  {
    ZEnable = false;

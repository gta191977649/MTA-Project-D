...
Pixel PixelShaderFunctionAO(PSInput PS)
{
    ...
    float3 viewNormal = normalize(GetNormalFromDepth(PS.TexCoord.xy, PS.UvToView));
...
Pixel2 PixelShaderFunctionBlur2(PSInput PS)
{
    Pixel2 output;
    ...
    output.World = float4(outColor.rgb, PS.Diffuse.a);
    output.Depth = FetchDepthBufferValue(PS.TexCoord.xy);
    return output;
}
...
// Techniques
//--------------------------------------------------------------------------------------
technique dxDrawPrimitive3D_ssao
{
  pass P0
  {
    ZEnable = false;
